module ScoresHelper
  ...
  #ghy
  def wrap_score2s_for_nvd3_chart(scores_2d_arr,training_class,current_user)
    ...
    scores_2d_arr.each do |arr_element|
      ...
      value_score= arr_element[1]
      if exam_type =='SAT'
        cr_scores      << {x:key_str,y:value_score.course_e_score}
        math_scores    << {x:key_str,y:value_score.course_b_score}
        writing_scores << {x:key_str,y:value_score.course_c_score}

        eassy_scores   << {x:key_str,y:value_score.course_a_score}

      end
    end

    if exam_type =='SAT'
      return_arr << {key:'Reading',values:cr_scores}  if can_view_subject('CR',training_class,current_user)
      return_arr << {key:'Math',values:math_scores}  if can_view_subject('Marh',training_class,current_user)
      return_arr << {key:'Writing',values:writing_scores}  if can_view_subject('Grammar',training_class,current_user)
      return_arr << {key:'Eassy',values:eassy_scores}
  ...
  #ghy
  def all_student_simulate_score2s_in_one_examation(exam_type,scores)
    return_2D_arr =[]
    #插入模考成绩
    scores.each do |score|
      total_score = score.total_score(exam_type)
      return_2D_arr<<  ["#{score.student.name}(#{score.course_e_score})",score]
    end
